...
GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwa0SxinMLAMVkKWn3gLJYWy1kDgazni_V_ccI2tbeT5B7Z5-u9gTSuIMchgKTZYuGMRw/exec"
...
async def reset_command(message: types.Message):
    ...
    try:
        ...
        try:
            ...
            await message.answer(
                "Критическая ошибка при выполнении /reset. Проверьте логи бота.",
                reply_markup=get_admin_keyboard()
            )
...
async def process_cancel_booking(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if callback.data == "admin_reset":
            ...
            # Очищаем кэш Google Script
            if MODE in ["GOOGLE", "HYBRID"]:
                clear_cache_result = clear_cache()
                if clear_cache_result['status'] == 'success':
                    print(f"[ADMIN] ✅ Кэш Google Script очищен")
                
                # Принудительно обновляем данные
                force_refresh_cache(callback.from_user.id)
            
            await callback.message.edit_text(
                "✅ *Кэш успешно сброшен и данные обновлены из Google Таблиц!*\n\n"
                "📌 *Важно:* Данные в Google Таблице не удалены.\n"
                "Для удаления всех записей используйте функции таблицы.",
                parse_mode="Markdown",
                reply_markup=get_admin_keyboard()
            )
            await callback.answer()
